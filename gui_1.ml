...
(* let draw_plr f = let bkg = read_bkg f in let p_xy = start_tile_one
   bkg in let img = Png.load "p1_fontile.png" [] in let g = of_image img
   in Graphics.draw_image g (fst p_xy) (snd p_xy) *)
...
(* let draw_obs1 ob = let img = Png.load "ob1_80.png" [] in let g =
   ...
   let draw_all_obs1 bkg = let obs1_lst = obs_one_xy bkg in List.iter
   draw_obs1 obs1_lst *)

(* let draw_all_obs2 bkg = let obs2_lst = obs_two_xy bkg in List.iter
   draw_obs2 obs2_lst
...
let player_moveup p (ob : obs) =
  if no_collision_up p ob then p.y <- p.y + 20;
  p

let player_movedown p ob =
  if no_collision_down p ob then p.y <- p.y - 20;
  p

let player_moveright p ob =
  if no_collision_right p ob then p.x <- p.x + 20;
  p

let player_moveleft p ob =
  if no_collision_left p ob then p.x <- p.x - 20;
  p
