...
(**[build_enemy pos off_tiles enemy_type] initializes an enemy of type
   [enemy_type] at position [pos], with [off_tiles] as tile he cannot step on*)
val build_enemy : int * int -> (int * int) list -> string -> t

(**[enemy_speed e] gets the current speed of enemy [e]*)
val enemy_speed : t -> int

(**[get_direction p] gets a list of directions in descending priority that the enemy should try based on player [p]*)
val get_direction : Player.t -> t -> direction list
(**[move_enemy dir_list] goes through [dir_list] and updates the enemy's position using the first direction without obstacles*)
val move_enemy : direction list -> Background.t -> t -> t
(**[collide_with_player p e] determines whether player [p] collides with enemy [e], and returns the updated player and enemy*)
